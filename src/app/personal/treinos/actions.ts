...
import { findById, insert, insertMany, list, remove, update } from "@/lib/nocodb/client";
...
export async function updateWorkout(payload: {
  ...
  const existing = await findById<{ coach_id: string; source: string }>(
    "workouts",
    payload.id,
    "coach_id,source",
  );
  if (!existing) return { error: "Treino não encontrado." };
  if (existing.coach_id !== session.sub) return { error: "Sem permissão." };
  ...
  // Substitui exercícios: pega IDs atuais e troca todos. session_sets já
  // mantém snapshot do exercise_name, então perder os IDs não quebra histórico.
  ...
  revalidatePath(`/personal/treinos/${payload.id}`);
  revalidatePath("/personal/treinos");
