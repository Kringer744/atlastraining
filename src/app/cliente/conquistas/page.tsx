...
export default async function Conquistas() {
  ...
  const [medalsRes, stats] = await Promise.all([
    ...
    safeFindOne<{ xp: number }>("client_stats", { where: `(client_id,eq,${session.sub})` }),
  ]);
  ...
  return (
    <AppShell title="Conquistas" bottomNav={<ClienteNav />}>
        ...
        {CATALOG.map((m) => {
          ...
          return (
            <div
              key={m.code}
              className={
                "atlas-card text-center " + (have ? "" : "opacity-50 grayscale")
              }
            >
