...
export default async function EvolucaoCliente() {
  ...
  const waterChart = waterDays.map((d) => ({
    ...d,
    value: d.value / 1000,
  }));

  // Volume últimos 14 sessões
  ...
  return (
    <AppShell title="Evolução" subtitle="Seus números reais" bottomNav={<ClienteNav />}>
      {/* Cards de overview */}
      <div className="grid grid-cols-2 gap-3 mb-4">
          ...
          {deltaWeight !== null && (
            <div
              className={
                ...
                (deltaWeight <= 0 ? "text-atlas-energy" : "text-red-300")
              }
            >
        ...
        <div className="atlas-card">
          ...
          <div className="text-2xl font-bold flex items-center gap-2">
            <Dumbbell className="text-atlas-energy" size={18} />
            {last30Days}
          </div>
          <div className="text-[11px] text-atlas-muted mt-1">
            de {totalSessions} no total
          </div>
        </div>
        <div className="atlas-card">
          <div className="text-[10px] uppercase tracking-wider text-atlas-muted">
          ...
          <div className="text-2xl font-bold flex items-center gap-2">
            <Droplet className="text-[#4FC3F7]" size={18} />
            {totalLitersWeek.toFixed(1)} L
          </div>
        </div>
      </div>
      ...
      {/* Volume últimos treinos */}
      {volSeries.length >= 2 && (
        <div className="atlas-card mb-3">
          <div className="text-[11px] uppercase tracking-wider text-atlas-muted mb-2">
            📊 Volume nos últimos treinos
          </div>
